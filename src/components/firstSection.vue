<template>
  <v-container fluid class="second">
    <v-layout class="layoutContainer" align-center justify-start>
      <v-flex xs12 sm10 md8 lg7>
        <div class="caption">
          <h2>Пальто inDresser</h2>
          <p>Оверсайз со спущенным рукавом</p>
        </div>
        <p>Уникальная вещь, которая не просто подчеркнет женственность, но еще и согреет этой холодной осенью. В чем же секрет этого незаменимого предмета гардероба?</p>
      </v-flex>
    </v-layout>
  </v-container>
</template>

<style scoped lang="stylus">
.second
  background-image url('/public/img/block2.jpg')
  background-size cover
  background-repeat no-repeat
  height 675px
  .layoutContainer
    height inherit
    .caption
      border-left 7px solid #A14F26
      padding-left 2rem
      margin-bottom 1.5rem
      color #A14F26
      h2
        line-height 4rem
      p
        font-size 2rem

</style>